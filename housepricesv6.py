...
df = pd.read_csv("LAlookup.csv")
earningdf = pd.read_csv("Medianincome.csv")
housedf = pd.read_csv("Medianhouseprice.csv")

years_list = list(earningdf.columns)
years_list.remove("LAD21NM")

years_options_list = [{"label": y, "value": y} for y in years_list]
for x in years_list:
    earningdf[x] = pd.to_numeric(earningdf[x], errors='coerce')
...
def yearregiondataframe(region, year):
    newdf = df[df["Region"] == region]
    earningdfbyyear = earningdf[["LAD21NM", year]]
    housedfbyyear = housedf[["LAD21NM", year]]

    df1 = pd.merge(newdf, earningdfbyyear, on='LAD21NM', how='left')
    df2 = pd.merge(df1, housedfbyyear, on='LAD21NM', how='left')

    df2['Ratio'] = df2.apply(lambda row: row[year + "_y"] / row[year + "_x"] if row[year + "_x"] != 0 else None, axis=1)
    df2.rename(columns={"LAD21NM": "Local Authority"}, inplace=True)
    df2['pct_rank'] = df2['Ratio'].rank(pct=True) * 100
    df2 = df2.round(1)
    return df2
...
region_options_list = [{"label": r, "value": r} for r in regions]
...
region_filter = html.Div([
    html.P("Selected region:", className="lead"),
    dcc.Dropdown(id='region_picker', options=region_options_list, value="England", clearable=False)
])

year_filter = html.Div([
    html.P("Selected year:", className="lead"),
...
breakdownbar = html.Div([
    ...
    html.P(id="click-output", className="lead"),
    html.Div(id="barchartcontainer", children=[
        dcc.Graph(id="barchart", style={'height': '40vh', 'width': '100%'}, config={'displayModeBar': False})
    ]),
    html.P("Local authorities with missing data have been excluded", style={'fontSize': 15})
])
...
app.layout = dbc.Container(fluid=True, children=[
    dbc.Row(dbc.Col(title, width=12), className="mb-2"),
    dbc.Row([
        dbc.Col(region_filter, md=2),
        dbc.Col(year_filter, md=2),
        dbc.Col(region_title, md=8)
    ...
    dbc.Row([
        dbc.Col(graph_bar, md=7, style={"padding": "10px"}),
        dbc.Col(breakdownbar, md=5, style={"padding": "10px"})
    ])
])
...
def display_choropleth(region, year):
    choropleth_df = yearregiondataframe(region, year)
    with open("LA_map.json") as json_file:
        geojson = json.load(json_file)
    ...
    centre_lat_long = {
        "England": {"lat": 52.567004, "lon": -1.180832},
        "East of England": {"lat": 52.238061, "lon": 0.571506},
        "West Midlands": {"lat": 52.523157, "lon": -2.189539},
        "East Midlands": {"lat": 52.871699, "lon": -0.679063},
        "South East": {"lat": 51.281418, "lon": -0.339193},
    ...
    fig.update_layout(
        ...
        mapbox_zoom=zoom_for_mapbox[region],
        mapbox_center=centre_lat_long[region],
        margin={"r": 0, "t": 0, "l": 0, "b": 0},
        uirevision='constant',
        coloraxis_colorbar=dict(yanchor="top", y=1, xanchor="left", x=1, dtick=5),
        clickmode='event+select'
    )

    return fig

@app.callback(Output("graph", "selectedData"), Input("region_picker", "value"))
def clear_selection_on_region_change(_):
    return None
...
def display_selected_data(selectedData, region, year):
    if selectedData is None:
        return "Click on a local authority to see further information"
    choropleth_df = yearregiondataframe(region, year)
    name = selectedData["points"][0]["location"]
    row = choropleth_df[choropleth_df["Local Authority"] == name]
    if row.empty:
        return "Click on a local authority to see further information"
    house = int(row[year + "_y"].values[0])
    income = int(row[year + "_x"].values[0])
    return f"In {year}, the median salary in {name} was £{income:,.0f}. At the same time, the average house price was £{house:,.0f}."

@app.callback(
    Output("click-output2", "children"),
    Input("graph", "selectedData")
)
def display_la_title(selectedData):
    if selectedData is None:
        return ""
    name = selectedData["points"][0]["location"]
    region_row = df[df["LAD21NM"] == name]
    if region_row.empty:
        return name
    return f"{name}, {region_row.iloc[0]['Region']}"
...
def display_percentile(selectedData, region, year):
    if selectedData is None:
        return ""
    choropleth_df = yearregiondataframe(region, year)
    name = selectedData["points"][0]["location"]
    ...
    if row.empty:
        return ""
    return f"Percentile rank in {region}: {int(row['pct_rank'].values[0])}"

@app.callback(
    Output("line_break", "hidden"),
    Input("graph", "selectedData")
)
def toggle_line_break(selectedData):
    return selectedData is None
...
def createbarchart(region, year, selectedData):
    if selectedData is None or year == "2010":
        return {}

    name = selectedData["points"][0]["location"]
    region_df = yearregiondataframe(region, year)
    if name not in region_df["Local Authority"].values:
        return {}

    region_df.rename(columns={"Local Authority": "LAD21NM"}, inplace=True)
    earningdf_2010 = earningdf[["LAD21NM", "2010"]]
    housedf_2010 = housedf[["LAD21NM", "2010"]]

    merged = pd.merge(region_df, earningdf_2010, on="LAD21NM", how="left")
    merged = pd.merge(merged, housedf_2010, on="LAD21NM", how="left")

    try:
        la_data = merged[merged["LAD21NM"] == name]
        la_earning_2010 = la_data["2010_x"].values[0]
        la_earning_year = la_data[year + "_x"].values[0]
        la_house_2010 = la_data["2010_y"].values[0]
        la_house_year = la_data[year + "_y"].values[0]

        la_earnings_pct = ((la_earning_year - la_earning_2010) / la_earning_2010) * 100
        la_house_pct = ((la_house_year - la_house_2010) / la_house_2010) * 100

        region_earnings_mean_2010 = merged["2010_x"].mean()
        region_earnings_mean_year = merged[year + "_x"].mean()
        region_house_mean_2010 = merged["2010_y"].mean()
        region_house_mean_year = merged[year + "_y"].mean()

        region_earnings_pct = ((region_earnings_mean_year - region_earnings_mean_2010) / region_earnings_mean_2010) * 100
        region_house_pct = ((region_house_mean_year - region_house_mean_2010) / region_house_mean_2010) * 100

        df_plot = pd.DataFrame({
            "metric": ["earnings", "house prices", "earnings", "house prices"],
            "percentage increase": [la_earnings_pct, la_house_pct, region_earnings_pct, region_house_pct],
            "geography": [name, name, region, region]
        })

        fig = px.bar(df_plot, x="metric", y="percentage increase", color="geography", barmode="group")
        fig.update_layout(title_text="<b>Percentage increase since 2010</b>", title_x=0.5, yaxis_title=None, legend_title="")
        return fig
